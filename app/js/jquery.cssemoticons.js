...
(function($) {
    $.fn.emoticonize = function(options) {
        var opts = $.extend({}, $.fn.emoticonize.defaults, options),
            ...
            specialEmoticons = { // emoticons to be treated with a special class, hash specifies the additional class to add, along with standard css-emoticon class
                ...
                '&lt;3':  { cssClass: 'heart-emoticon counter-rotated' },
            ...
            preMatch = '(^|[\\s\\0])',
            emoticon, i, exclude, excludeArray;

        for(i = threeCharacterEmoticons.length - 1; i >= 0; --i) {
        ...
        for(i = twoCharacterEmoticons.length - 1; i >= 0; --i) {
        ...
        for(emoticon in specialEmoticons) {
            if(specialEmoticons.hasOwnProperty(emoticon)) {
        ...
        return this.not(exclude).each(function() {
            ...
            for(emoticon in specialEmoticons) {
                if(specialEmoticons.hasOwnProperty(emoticon)) {
                    ...
                    container.html(container.html().replace(specialEmoticons[emoticon].regexp, '$1<span class="' + specialCssClass + '"><span>$2</span></span>'));
                }
            }
